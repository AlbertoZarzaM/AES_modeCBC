...
void printMatrix1(std::vector<std::vector<std::string>> state);
...
std::vector<std::vector<std::string>> modeCBC (std::vector<std::vector<std::string>> state1, std::vector<std::vector<std::string>> state2, std::vector<std::vector<std::vector<std::string>>> key, std::vector<std::vector<std::string>> IV){
  
  std::vector<std::vector<std::string>> resultBlock1 (4, std::vector<std::string>(4));
  std::vector<std::vector<std::string>> resultBlock2 (4, std::vector<std::string>(4));
  //XOR entre el bloque 1 de texto y el IV
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      state1[i][j] = hexadecimalXOR(state1[i][j], IV[i][j]);
    }
  }

  //Primera ronda
  resultBlock1 = AES(state1, key);

  //XOR entre el bloque 2 de texto y el bloque 1 cifrado
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      state2[i][j] = hexadecimalXOR(state2[i][j], resultBlock1[i][j]);
    }
  }

  //Segunda ronda
  resultBlock2 = AES(state2, key);

  //Mostrar resultado
  std::cout << "Bloque 1 cifrado: " << std::endl;
  printMatrix1(resultBlock1);
  std::cout << "Bloque 2 cifrado: " << std::endl;
  printMatrix1(resultBlock2);
  return resultBlock2;
}


std::vector<std::vector<std::string>> modeCBCstealing (std::vector<std::vector<std::string>> state1, std::vector<std::vector<std::string>> state2, std::vector<std::vector<std::vector<std::string>>> key, std::vector<std::vector<std::string>> IV){

  std::vector<std::vector<std::string>> resultBlock1 (4, std::vector<std::string>(4));
  std::vector<std::vector<std::string>> resultBlock2 (4, std::vector<std::string>(4));
  std::vector<std::vector<std::string>> resultBlock1aux (4, std::vector<std::string>(4));
  std::vector<std::vector<std::string>> resultBlock2aux (4, std::vector<std::string>(4));
  //XOR entre el bloque 1 de texto y el IV
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      state1[i][j] = hexadecimalXOR(state1[i][j], IV[i][j]);
  ...
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      resultBlock2[i][j] = hexadecimalXOR(resultBlock1[i][j], state2[i][j]);
    }
  }
  //Segunda ronda
  for (int i = 0; i < 4; i++){
    for (int j = 0; j < 4; j++){
      ...
      if (state2[i][j] == "--"){
        resultBlock1aux[i][j] = "--";
        resultBlock2[i][j] = resultBlock1[i][j];

      }
      else {
        resultBlock1aux[i][j] = resultBlock1[i][j];
      } 
    }
  }

  resultBlock2aux = AES(resultBlock2, key);
  //XOR entre el bloque 2 de texto y el bloque 1 cifrado

  std::cout << "Bloque 1 cifrado: " << std::endl;
  printMatrix1(resultBlock2aux);

  std::cout << "Bloque 2 cifrado: " << std::endl;
  printMatrix1(resultBlock1aux);
